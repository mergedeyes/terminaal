...
pub fn section_title(ui: &mut Ui, text: &str) {
    ui.label(RichText::new(text.to_uppercase()).size(11.0).strong().color(theme::TEXT_WEAK));
}

pub fn weak(text: impl Into<String>) -> RichText {
    RichText::new(text).color(theme::TEXT_WEAK)
}
...
pub fn list_row(ui: &mut Ui, title: &str, subtitle: &str, selected: bool, badge: Option<&str>, hint: &str) -> egui::Response {
    ...
    if selected {
        painter.rect_filled(rect, CornerRadius::same(4), theme::SELECTED_BG);
        painter.rect_filled(Rect::from_min_size(rect.min, vec2(3.0, rect.height())), CornerRadius::same(2), theme::ACCENT);
    } else if response.hovered() {
        painter.rect_filled(rect, CornerRadius::same(4), theme::HOVER_BG);
    }
    let x = rect.left() + 12.0;
    painter.text(pos2(x, rect.top() + 13.0), Align2::LEFT_CENTER, title, FontId::proportional(14.0), theme::TEXT);
    painter.text(pos2(x, rect.top() + 30.0), Align2::LEFT_CENTER, subtitle, FontId::monospace(11.0), theme::TEXT_WEAK);
...
impl Status {
    ...
    pub fn show(&self, ui: &mut Ui) {
        let color = if self.error { theme::ERROR } else { theme::SUCCESS };
